...
from .utils import exists, lens_to_mask
...
class CFMEdit(nn.Module):
    ...
    def sample(
        ...
        *,
        ...
        return self.sample_from_embeddings(
            cond,
            text_embeds,
            context_mask,
            duration,
    ...
    def sample_from_embeddings(
        ...
        *,
        ...
        transformer_devices = {parameter.device for parameter in self.transformer.parameters()}
        if transformer_devices != {device}:
            raise RuntimeError(f"AuK transformer device mismatch before sampling: {sorted(map(str, transformer_devices))}, expected {device}")
