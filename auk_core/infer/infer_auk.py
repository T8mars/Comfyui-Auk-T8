...
class AukInfer:
    def __init__(
        ...
        *,
        ...
        self.defer_to_cpu = defer_to_cpu
        ...
        if cpu_offload and defer_to_cpu:
            raise ValueError("cpu_offload and defer_to_cpu cannot be enabled together.")
        ...
        transformers_minor = tuple(int(part) for part in transformers_version.split(".", maxsplit=2)[:2])
        dtype_keyword = "dtype" if transformers_minor >= (4, 57) else "torch_dtype"
        thinker = Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained(
            text_encoder_config.text_encoder_path,
            low_cpu_mem_usage=True,
            **{dtype_keyword: self.dtype},
        )
    ...
    def _load_ema_weights(self, model: CFMEdit, ckpt_path: str):
        ...
        if not ckpt_path.endswith(".safetensors"):
            raise ValueError("Native ComfyUI nodes only accept safetensors AuK checkpoints")
